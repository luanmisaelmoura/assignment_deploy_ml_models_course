...
def predict(data):
    ...
    # Group rare labels
    for var in config.CATEGORICAL_VARS:
        pf.remove_rare_labels(data, var, config.FREQUENT_LABELS[var])
    
    # encode variables
    data = pf.encode_categorical(data, config.ENCODER_PATH)

    # check all dummies were added
    pf.check_dummy_variables(data, config.DUMMY_VARIABLES)
    
    # scale variables
    pf.scale_features(data, config.OUTPUT_SCALER_PATH)
    
    # make predictions
    predictions = \
        pf.predict(data, config.OUTPUT_MODEL_PATH)
